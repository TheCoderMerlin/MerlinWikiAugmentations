...
class Topics {
     ...
     * @returns {undefined}
     */
    static async setMissionHref($mission, missionId) {
        ...
        getJson(baseUrl + path).then((data) => {
     ...
     */
    static buildMissionName(mission) {
     ...
     * @returns {Promise}
     */
    static async handleTopicPress(event) {
        ...
        missions.sort((left, right) => {
            // Build the names
            const leftMissionName = Topics.buildMissionName(left);
            const rightMissionName = Topics.buildMissionName(right);
        ...
        for (let i = 0; i < missions.length; i++) {
            ...
            // Add to the list
            let $li = $(`<li>${Topics.buildMissionName(mission)}</li>`);

            // Set the href
            Topics.setMissionHref($li, `M${mission['sequence']}-${mission['suffix']}`);
     ...
     * @param  {string} selector The CSS selector identifying a topic name
     * @returns {undefined}
     */
    static addTopicHandlers($table, selector) {
        // Iterate over each of the topic names
        $table.find(selector).click(Topics.handleTopicPress);
    }
}

export default Topics.addTopicHandlers;